...
class Company(AggregateRoot):
    ...
    def get_appointments(self, person: Person):
        return [officer for officer in self._officers if officer.person == person]
    ...
    def directors(self) -> List:
        return [officer.person for officer in self._officers if officer.officer_type == "director"]
    ...
    def secretary(self) -> bool:
        return [sec.person for sec in self._officers if sec.officer_type == "secretary"][0]
    ...
    def get_shareholdings(self, person = None, share_class_name = None) -> List:
        if share_class_name:
            ...
            assert isinstance(share_class, ShareClass)
            if person:
                return [holding for holding in share_class.shareholdings if holding.shareholder == person]
...
class Officer(object):
    ...
    def __repr__(self):
        return f"<Officer(name='{self.person.name}', officer_type='{self.person.officer_type}')>"
...
class Shareholding(object):
    ...
    def __repr__(self):
        return f"<Shareholding(shareholder='{self.shareholder.name}', number_of_shares={self.number_of_shares}>"
    ...
    def certificate(self):
        return "Share certificate: {name} of {address} holds {no_shares} {share_class} shares of {curr} £{nom_val} each in the capital of {co_name}".format(
            name=self.shareholder.name,
            address=self.shareholder.address,
